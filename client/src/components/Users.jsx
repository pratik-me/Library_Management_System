      ...
      <main className="relative flex-1 p-6 pt-28">
        ...
        {/* TABLE */}
        {
          users && users.filter(u => u.role === "User").length > 0 ? (
              ...
              <table className="min-w-full border-collapse">
                <thead>
                  <tr className="bg-gray-200">
                    ...
                    <th className="px-4 py-2 text-center">Number of Books</th>
                ...
                <tbody>
                  {
                    users.filter(u => u.role === "User").map(
                      (user, index) => (
                        <tr key={user.id} className={(index + 1) % 2 === 0 ? "bg-gray-50" : ""}>
                          <td className="px-4 py-2">{index + 1 % 2 === 0}</td>
            ...
            <h3 className="text-3xl mt-5 font-medium"></h3>
          )
        }
      </main>
    </>
